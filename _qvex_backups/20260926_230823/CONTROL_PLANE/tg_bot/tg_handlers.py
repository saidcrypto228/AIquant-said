...
def get_state() -> dict:
    ...
    try:
        ...
        # Если пришел канонический CoreState (GAP-04) — мапим поля для существующих шаблонов
        if "schema_version" in raw:
            eq = raw.get("equity")
            free = raw.get("free_margin")
            ...
            for p in positions_raw:
                pos_adapted.append({
                    ...
                    "side": p.get("side"),
                    "size": p.get("size", 0.0),
                    "entry_price": p.get("entry_px", 0.0),
                    "current_price": p.get("highest_px") or p.get("entry_px", 0.0),
                    "sl_price": p.get("sl_px", 0.0),
                    "pnl_usd": 0.0,
                    "pnl_pct": 0.0,
                    "is_breakeven": p.get("breakeven_active", False),
                    "trailing_active": p.get("trailing_active", False)
                })

            raw_regime = raw.get("market_regime") or "BULL"
            status_reg = "BULL" if "BULL" in str(raw_regime).upper() else "DEFENSIVE"

            return {
                "account": {
                    "equity": eq if eq is not None else 0.0,
                    "total_balance": eq if eq is not None else 0.0,
                    "free_margin": free if free is not None else 0.0,
                    "unrealized_pnl": pnl if pnl is not None else 0.0,
                    "is_dry_run": eq is None
                },
                "market_regime": {
                    "status": status_reg,
                    "slope": 0.0,
                    "storm_filter_active": False,
                    "btc_price": raw.get("btc_price", 0.0)
                },
                "positions": pos_adapted,
                "system_status": raw.get("system_status", "ACTIVE"),
                "trading_enabled": raw.get("trading_enabled", True)
            }
        return raw
    except Exception:
        return {}
...
async def cb_panic_confirm(q: CallbackQuery):
    ...
    ctrl_mgr = ControlStateManager(filepath=str(settings.control_path))
    ctrl_mgr.trigger_panic(admin_tag=f"tg_admin_{q.from_user.id}")
