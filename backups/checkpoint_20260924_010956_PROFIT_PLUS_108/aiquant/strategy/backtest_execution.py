    ...
    *,
    ...
    while i < len(close) - 1:
        ...
        for j in range(entry_idx, exit_idx + 1):
            ...
            # 2. Динамический параболический распад TP (Decaying TP)
            # С 12 по 24 бар целевой тейк снижается навстречу затухающему импульсу
            stagnation_bar = 12
            min_tp_buffer = max(risk_dist * 0.25, plan.entry * 0.0025) # Гарантированное покрытие комиссий + 0.15%
            ...
            # 3. Отсечение стагнации отключено: сделка дышит полные 24 бара, 
            # а Decaying TP снижается навстречу цене.

            # 3b. Институциональный Time-Decay Stop (защита от дрейфа после 4 часов)
            # Если сделка сидит в рынке 16+ баров и находится в явном минусе (R <= -0.25),
            # фиксируем минимальный убыток, не дожидаясь сползания до -0.75R на 24-м баре.
            # Раннее отсечение токсичного дрейфа: проверка с 12-го бара (3 часа)
            if bars_held >= 12:
        ...
        # Институциональный кулдаун (Execution Cooldown): 8 баров (2 часа) паузы
        # после закрытия позиции для защиты от немедленного распила на остаточном шуме
        cooldown_bars = 8
        next_i = min(exit_idx + 1 + cooldown_bars, len(close) - 1)
