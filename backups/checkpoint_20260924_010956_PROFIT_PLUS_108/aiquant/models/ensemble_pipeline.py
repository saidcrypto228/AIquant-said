...
def run_ml_backtest(df: pd.DataFrame, pair: str, capital: float = 100_000,
    ...
    # Бычий макро-рынок: цена выше 50-дневной EMA
    ...
    if 'mtf_4h_structure_regime' in df.columns:
        ...
        # Лонги: структура 4H вверх И цена выше 50-дневного тренда
        bullish_regime = (struct > 0) & macro_bull
        # Шорты: структура 4H вниз И цена СТРОГО ниже 50-дневного тренда И ниже 4H EMA
    ...
    candidates = [0.58, 0.60, 0.62, 0.64]
    best_long = 0.62
    best_short = 0.62
    ...
    LONG_HARD_THRESHOLD = max(best_long, 0.62)
    SHORT_HARD_THRESHOLD = max(best_short, 0.62)
    ...
    # Шорты: облагаются надбавкой +0.06 в бычьем режиме
    short_thresh_effective = np.where(bullish_regime, SHORT_HARD_THRESHOLD + SHORT_REGIME_PENALTY, SHORT_HARD_THRESHOLD)
    short_mask = (oos_p0 >= short_thresh_effective) & ((oos_p0 - oos_p1) >= FLAT_MARGIN)
    sig[short_mask] = -1
