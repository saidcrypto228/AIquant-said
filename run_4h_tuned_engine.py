#!/usr/bin/env python3
"""
Refined 4H Swing Trend Engine (2023-2024).
- Universe: SOL, NEAR (Clean momentum leaders, ETH & SUI excluded).
- Rule: 1 asset = max 1 position (No duplicate stacking).
- BE Shield: Move SL to Entry + Fees once Price >= Entry + 1.2 ATR.
- Trailing exit on 4H candle close below EMA20.
- Sizing: 1.5% equity risk.
"""
...
RISK_PCT = 0.015
SL_MULT = 1.80
BE_TRIGGER_ATR = 1.20   # Сдвиг стопа в БУ при проходе 1.2 ATR
...
print("=" * 65)
print("  ФОКУС-ТЕСТ: СВИНГ-МОМЕНТУМ SOL & NEAR (4H СВЕЧИ)")
print("=" * 65)
...
for dt_idx, cur_dt in enumerate(unique_datetimes):
    ...
    for order in pending_orders:
        ...
        # Если позиция по монете уже открыта — отменяем ордер
    ...
    for pos in open_positions:
        ...
        # Проверка триггера безубытка
        if not pos["be_activated"] and cur_h >= pos["entry_price"] + (pos["atr"] * BE_TRIGGER_ATR):
            pos["sl_price"] = pos["entry_price"] * (1.0 + FEE_MAKER_PCT + FEE_TAKER_PCT + 0.001)
        ...
        if cur_l <= pos["sl_price"]:
            exit_p = pos["sl_price"]
            reason = "BE_SHIELD" if pos["be_activated"] else "SL_WIDE"
    ...
    # 3. Выбор новых ордеров (только если монеты нет в позициях)
...
print("=" * 65)
print("  РЕЗУЛЬТАТЫ СВИНГ-МОДЕЛЕЙ ДЛЯ РЕАЛЬНОГО ДЕПОЗИТА (SOL & NEAR)")
print("=" * 65)
...
df_trades.to_csv(results_dir / "refined_swing_trades.csv", index=False)
print("=" * 65)
