#!/usr/bin/env python3
"""
Asymmetric Multi-Asset Engine (70/30 TP Tranche & $25 Risk).
Optimized for prop-firm survival: locking the lion's share of profits on TP1
and extending maximum drawdown buffer.
"""
...
# Новые риск-параметры
RISK_USD = 25.0          # 0.25% на сделку ($25 от $10,000)
TP1_RATIO = 0.70        # 70% закрываем на TP1
TP2_RATIO = 0.30        # 30% держим до TP2
...
print(f"[*] Запуск OOS симулятора 70/30 с риском $25 ({len(unique_test_ts):,} баров)...")
    ...
    for pos in open_positions:
        ...
        if not pos["tp1_hit"]:
            ...
            if hit_tp1:
                ...
                pnl1 = (pos["pos_usd"] * TP1_RATIO) * (ret1 - FEE_PCT)
                equity += pnl1
                daily_pnl += pnl1
                pos["pnl_accum"] += pnl1
                # Перевод стопа оставшихся 30% в безубыток
                pos["sl_price"] = pos["entry_price"] * (1.0 + (FEE_PCT if side == 1 else -FEE_PCT))

        # 2. Проверка выхода оставшейся части
        ...
        if exit_remaining:
            rem_fraction = TP2_RATIO if pos["tp1_hit"] else 1.0
...
print("=" * 65)
print("  РЕЗУЛЬТАТЫ OOS БЭКТЕСТА: ТЕЙК 70/30 И РИСК $25")
print("=" * 65)
...
if not df_trades.empty:
    df_trades.to_csv(results_dir / "universe_asymmetric_trades.csv", index=False)

plt.figure(figsize=(12, 6))
plt.subplot(2, 1, 1)
plt.plot(eq_arr, label="Portfolio Equity ($)", color="#2ca02c", lw=1.5)
plt.axhline(INITIAL_CAPITAL, color="gray", linestyle="--", alpha=0.7)
plt.title("Hyperliquid Portfolio Equity (70/30 TP & $25 Risk)")
plt.ylabel("Balance ($)")
...
chart_path = results_dir / "universe_asymmetric_chart.png"
plt.tight_layout()
